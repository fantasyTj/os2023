...
void syscallVar(struct StackFrame *sf);
...
void syscallReadStdIn(struct StackFrame *sf) {
	// TODO: complete `stdin`
	if (dev[STD_IN].value == 0) {  // no preocess blocked
		dev[STD_IN].value --;
		...
		asm volatile("movb $0x00, %%es:(%0)"::"r"(str+i)); // the end of str
...
int var;
void syscallVarInit(struct StackFrame *sf);
