...
pid_t fork() {
	pid_t id = syscall(SYS_FORK, 0, 0, 0, 0, 0);
	// printf("%d\n", id);
	return id;
}
...
int var_init(int init_var) {
	return syscall(SYS_VAR, VAR_INIT, init_var, 0, 0, 0);
}

int var_change(boolean add, int num) {
	return syscall(SYS_VAR, VAR_CHANGE, add, num, 0, 0);
}